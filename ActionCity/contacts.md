...
- **fallback:** owner DM — `lark_send_message` with `receive_id=vichit@sfb.co.th`, `receive_id_type=email` (no second group identified; `lark_batch_get_user_id` could not resolve an open_id for this address, so address by email directly).
