...
- **chat_id:** `oc_4dabe0f3436e1201813c8cea6e38dbb1` (ActionCity TH — resolved via `lark_list_chats`).
- **fallback:** owner DM by email, `receive_id_type=email`, `receive_id=vichit@sfb.co.th` (open_id lookup via `lark_batch_get_user_id` returned not-found/no-permission, so email routing is used instead).
