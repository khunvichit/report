...
def main():
    ...
    # Strip the template's leading how-to comment header (the one before <!DOCTYPE),
    # then any leftover REPEAT/SECTION markers.
    html = re.sub(r"^\s*<!--(?!\s*(?:REPEAT|SECTION):).*?-->\s*", "", html, count=1, flags=re.DOTALL)
